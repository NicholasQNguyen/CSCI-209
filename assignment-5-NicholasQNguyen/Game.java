...
public class Game extends JFrame implements KeyListener {
	...
	public void animate() {
		...
		//variable used to stop the diamond form being drawn every frame
		int diamondCounter = 0;

		// our images
		professor = new Human(XBOUND - 50, YBOUND - 50, brush);
		goblin = new Goblin(0, YMIN_DISPLAY, brush);
		treasure = new Treasure(rand.nextInt(800), rand.nextInt(580)+20, brush);
		//array holding our images
		GamePiece[] gamePieces = new GamePiece[3];
		gamePieces[0] = professor;
		gamePieces[1] = goblin;
		gamePieces[2] = treasure;
		...
		while (!stop) {
			...
			*/
			...
			nextFrame(FDELAY);
			/**
			//TODO: Check for victoory/defeat
			if(professor.overlaps(treasure))
			{
				win = true;	
				break;
			}
			if (professor.overlaps(goblin))
			{
				break;
			}
			*/
